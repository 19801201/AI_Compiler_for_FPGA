...
class YoloLoss(nn.modules.loss._Loss):
    ...
    def forward(self, output, target):
        ...
        if torch.cuda.is_available():
            pred_boxes = pred_boxes.cuda(1)
            lin_x = lin_x.cuda(1)
            lin_y = lin_y.cuda(1)
            anchor_w = anchor_w.cuda(1)
            anchor_h = anchor_h.cuda(1)
        ...
        if torch.cuda.is_available():
            tcoord = tcoord.cuda(1)
            tconf = tconf.cuda(1)
            coord_mask = coord_mask.cuda(1)
            conf_mask = conf_mask.cuda(1)
    ...
    def build_targets(self, pred_boxes, ground_truth, height, width):
        ...
        tconf = torch.zeros(batch_size, self.num_anchors, height * width, requires_grad=False).cuda(1)
        for b in range(batch_size):
            ...
            gt = torch.zeros(len(ground_truth[b]), 4).cuda(1)
            ...
            gt_wh = gt.clone()
            gt_wh[:, :2] = 0
            iou_gt_anchors = bbox_iou(gt_wh, anchors.cuda(1))
